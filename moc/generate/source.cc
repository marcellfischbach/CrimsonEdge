...
std::string ClassGenerator::GenerateFunctionVoidInvokeMethod(ClassNode *classNode, FunctionNode *function, bool constMethod)
{
  std::string fkt;
  ...
  {
    fkt += "    " + sConst + classNode->GetName() + " *d = ce::QueryClass<" + sConst + classNode->GetName() + ">(obj);\n";
    fkt += "    if (!d)\n";
    fkt += "    {\n";
    fkt += "      throw ce::BadMethodInvokation(\"[" + function->GetName() + "] Object is not instance of " + classNode->GetName() + "\");\n";
    fkt += "    }\n\n";
...
std::string ClassGenerator::GenerateFunctionValueInvokeMethod(ClassNode *classNode, FunctionNode *function, bool constMethod)
{
  std::string fkt;
  std::string sConst = constMethod ? "const " : "";
...
std::string ClassGenerator::GenerateFunctionReferenceInvokeMethod(ClassNode *classNode, FunctionNode *function, bool constReturn, bool constMethod)
{
  std::string fkt;
  std::string sConst = constMethod ? "const " : "";
  ...
  fkt += "\n";
  fkt += "  virtual " + rConst + "void *Invoke" + mConst + "ReferenceImpl (" + sConst + "ce::iObject* obj, ...) const\n";
  fkt += "  {\n";
...
std::string ClassGenerator::GenerateFunctionPointerInvokeMethod(ClassNode *classNode, FunctionNode *function, bool constReturn, bool constMethod)
{
  std::string fkt;
  std::string sConst = constMethod ? "const " : "";
...
std::string ClassGenerator::GenerateAttribute(ClassNode* classNode, std::list<NamespaceNode*>& nss, MemberNode* member, const std::string& visibility, CSMetaNode* meta)
{
  ...
  prop += "public:\n";
  prop += "  " + propClassName + "()\n";
  prop += "    : ce::Property(" + GenerateCSValueDeclaration(containerType, false) + ", \"" + propName + "\", " + GenerateCSValueDeclaration(type, true) + ")\n";
  prop += "  {\n";
  prop += "  }\n\n";
  ...
  if (visibility == std::string("public") && !type.IsConst())
  {
    ...
    {
      prop += "    d->" + member->GetName() + " = *reinterpret_cast<" + GenerateInputTypeForAttribute(type) + ">(data);\n";
    }
    else
    {
  ...
  {
    prop += "    throw ce::BadMethodInvokation(\"Property [" + classNode->GetName() + "::" + member->GetName() + "] is not public\");\n";
  }
  prop += "  }\n\n";
...
std::string ClassGenerator::GenerateQueryClass(ClassNode * classNode, std::list<NamespaceNode*> & nss, CSMetaNode *, bool _const)
{
  std::string fns = Generator::GetFullNamespaceName(nss);
